...
# ========================================================
st.set_page_config(
    page_title="SARIMAX Exogenous ES + ML Strategy",
    page_icon="🤖",
    layout="wide",
...
def exog_feature_diagnostics(feat_df, prices, horizon=1):
    """
    Compute simple diagnostics for SARIMAX exogenous features:
    - Correlation with next return
    - t-stat from univariate regression
    - Directional hit rate
    """
    import statsmodels.api as sm

    future_ret = np.log(prices).diff(horizon).shift(-horizon)
    ...
    for col in EXOG_CANDIDATES:
        ...
        # Directional hit rate
        hit_rate = np.mean(
            np.sign(x_clean) == np.sign(y_clean)
        )
    ...
    return pd.DataFrame(results).sort_values(
        by="T-Stat", key=np.abs, ascending=False
    )
...
st.markdown(
    ...
    <div style="padding: 1rem 1.25rem; border-radius: 0.9rem;
      ...
      <div style="display:flex; justify-content:space-between; align-items:center;">
        <div>
<h2>SARIMAX Exogenous ES + ML Hybrid Strategy</h2>
<p>
Dynamic regression (SARIMAX) forecasts next-period returns using
exogenous features, then filters the α–β Exponential Smoothing strategy.
</p>
...
exog_full = select_exog_features(feat_df).dropna()

# Align everything
common_idx = log_returns.index.intersection(exog_full.index)

endog = log_returns.loc[common_idx]
exog  = exog_full.loc[common_idx]

st.markdown("## 🧪 Exogenous Feature Diagnostics (SARIMAX)")

exog_diag = exog_feature_diagnostics(feat_df, prices)

st.write(
    "These diagnostics help evaluate which features are suitable "
    "as exogenous regressors for SARIMAX."
)
...
st.markdown("### 📈 SARIMAX Exogenous Feature Selection")

default_exog = exog_diag["Feature"].head(4).tolist()

selected_exog = st.multiselect(
    "Select exogenous features for SARIMAX",
    options=exog_diag["Feature"].tolist(),
    default=default_exog,
    help="Choose features based on diagnostics above"
)
